...
void data_types_and_decls() {
    ...
    bool b;
    char c;
    int *y = &x; // address-of operator (it adds a * to the type)
...
void exprs() {
    ...
    // e.g., int z = x++; vs. int z = ++x; do different things
...
void exprs2() {
    ...
    cout << x++ << endl; // change x to 7, and print out 6
    cout << ++x << endl; // change x to 8, and print out 8
}
...
87 (base 10) is what in hex? binary?
...
...
int my_array[] = {1, 2, 3, ...};
array_twice(my_array, size);
*/

// Pointers

int *global_pointer_variable;
...
void pointers() {
    ...
    *p = 5; // x is now 5 because x and *p are linked together
    ...
    // here's an example of saving y into a global variable
    global_pointer_variable = y;
}
...
// laid out in memory contiguously in the order declared
struct Point {
    int x;
    int y;
};
...
void structs() {
    ...
    Point *pp = &p2;
    pp->y = 5; // equivalent to (*pp).y = 5; (and also equiv. to p2.y = 5;)
...
void dynamic_memory_allocation() {
    ...
    *y = 7;
    ...
    delete y;
    // y = nullptr; <-- it's a good idea to do this to make sure you don't access a freed memory location
    // using *y after this is undefined (until you set y to be some valid address)
...
// compute n*m -- this is a bit similar to the power function, isn't it?
// precondition: m >= 1
...
// base case: free all the nodes in an empty list--do nothing!
// recursive case: free the smaller list
...
// base case: print all the nodes in an empty list--do nothing!
// recursive case: print our head, then recursively print the smaller list
void print_list_forwards(Node *head) {
    if (head == NULL) return;

    cout << head->data << endl;
    print_list_forwards(head->next);
}

// base case: print all the nodes in an empty list--do nothing!
// recursive case: recursively print the smaller list backwards, and then print our head
